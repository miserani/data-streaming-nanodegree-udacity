...
def run_server():
    ...
    if topic_check.topic_exists("TURNSTILE_SUMMARY") is False:
        ...
        exit(1)
    if topic_check.topic_exists("com.transitchicago.station.tables") is False:
        logger.fatal(
    ...
    consumers = [
        KafkaConsumer(
            "com.transitchicago.station.weather",
            weather_model.process_message,
            offset_earliest=True,
        ),
        KafkaConsumer(
            "com.transitchicago.station.tables",
            lines.process_message,
            offset_earliest=True,
            is_avro=False,
        ),
        KafkaConsumer(
            "^com.transitchicago.station.arrivals.",
            lines.process_message,
            offset_earliest=True,
        ),
        KafkaConsumer(
